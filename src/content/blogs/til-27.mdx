...
<Aside>
  Since writing this blog post, due to API changes to expressive-code, I was able to create a [plugin](https://github.com/DerTimonius/ec-magic-move) to run the animations directly in expressive-code.

I have changed the animations here to use the plugin and not the Svelte component. The text is still valid, but the plugin has a better performance.
...
```js hide-badge magic-move-before={1-3} magic-move-after={4-8}
...
```js hide-badge magic-move-before={1-12} magic-move-after={13-37}
export function animateCodePlugin() {
	return {
		name: 'Animate Code',
		hooks: {
...
function getCorrectBlocks(lines) {
	const beforeStartingIdx =
		lines.findIndex((line) => line.text === '<!-- before -->') + 1;
	const afterStartingIdx =
		lines.findIndex((line) => line.text === '<!-- after -->') + 1;
...
```svelte hide-badge magic-move-before={1-12} magic-move-after={13-55}
<pre bind:this={container} class="shiki-magic-move-container">
  {#if !isMounted}
    {#each props.tokens.tokens as token}
      {#if token.content === '\n'}
...
Looking good! Let's get the correct code in here with props and change the logic a bit. And also, create a good looking button and move it to the same place the copy button is placed with `expressive-code`.

```svelte hide-badge magic-move-before={1-22} magic-move-after={23-81}
<script lang="ts">
  import { getHighlighter } from 'shiki';
  import MagicMove from './MagicMove.svelte';
      ...
      >
        <path
          ...
          d="M5.25 5.653
            ...
            1.667-.986
            l11.54 6.347
            a1.125 1.125 0 0 1 0 1.972
            l-11.54 6.347
            a1.125 1.125 0 0 1-1.667-.986
            V5.653
            Z"
        />
      </svg>
...
<Aside>
	The Autoanimation feature has not been added to the plugin, but it is planned.
</Aside>
...
```svelte hide-badge magic-move-before={1-28} magic-move-after={29-82}
<script lang="ts">
import { getHighlighter } from 'shiki';
import MagicMove from './MagicMove.svelte';
...
```ts hide-badge magic-move-before={1-17} magic-move-after={18-40}
const observer = new IntersectionObserver(
	(entries) => {
		entries.forEach((entry) => {
			if (entry.isIntersecting) {
