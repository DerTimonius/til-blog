...
<Aside>
	I maybe should mention that I have released another CLI tool called
	[**twkb**](https://github.com/DerTimonius/twkb) and that I have written it in
	Go. Once I added the project view, I will write a post!
</Aside>
...
```ts hide-badge magic-move-before={1-5} magic-move-after={6-11}
const users = await prisma.user.findUnique({
	where: {
		id: input.id,
	},
...
```ts hide-badge magic-move-before={1-6} magic-move-after={7-12}
const users = await prisma.user.findUnique({
	include: { posts: true },
	where: {
		id: input.id,
...
And that was a problem. Alacritty does not support the _kitty image protocol_ that allows rendering of images in the terminal. So if I want to use the terminal to its fullest, there are several options:
