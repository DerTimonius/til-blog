  ...
  {#each foundPosts as foundPost}
    <li class="my-6">
      <a
        ...
        class="inline-block text-lg font-medium text-skin-accent decoration-dashed underline-offset-4 focus-visible:underline-offset-0"
      ...
      {#each foundPost.item.tags as tag}
        <li class="inline-block my-1 underline-offset-4">
          <svg
            ...
            class="-mr-5 h-6 w-6 text-skin-base opacity-80 scale-75"
            ><path
    ...
    <div
      class="max-w-4xl mx-auto px-2 border-dashed border-skin-light border-b-2 last:hidden"
    />
  {/each}
</ol>
